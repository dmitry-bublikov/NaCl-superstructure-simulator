...
const int COMP_C = 1;         // number of cation components (including vacancy)
...
{
    int st_c1;     // stoichiometry for each cation
    uint8_t ch_c1; // cation charges
...
// **************************************************************************************************
    ...
    for (int skip7 = skip6 + 1; skip7 < 8; skip7++) {
        int idx = 0;
        for (int i = 0; i < 8; i++)
            if (i != skip1 && i != skip2 && i != skip3 && i != skip4 && i != skip5 && i != skip6 && i != skip7)
      ...
      for (st_c1 = 1; st_c1 <= diff_c; st_c1++)
    {
        ...
        printf("Processing st_c1 = %d from %d var = %lld  [%02d:%02d:%02d]\n", st_c1, diff_c, var, hours, minutes, secs);
            ...
            // total stoichiometry for cations =cf
            if (st_c1!=cf) continue;
