...
const int COMP_C = 3;         // number of cation components (including vacancy)
...
{
    int st_c1, st_c2, st_c3;     // stoichiometry for each cation
    uint8_t ch_c1, ch_c2, ch_c3; // cation charges
...
// **************************************************************************************************
    ...
    int all_charges[] = {0, 1, 2, 3, 4, 5, 6, 7};
    int combo[COMP_C];
    ...
    for (int skip5 = skip4 + 1; skip5 < 8; skip5++) {
        int idx = 0;
        for (int i = 0; i < 8; i++)
            if (i != skip1 && i != skip2 && i != skip3 && i != skip4 && i != skip5)
               combo[idx++] = all_charges[i];
        do {
            for (int k = 0; k < COMP_C; k++)
                charges_c[charge_count_c][k] = combo[k];
            charge_count_c++;
        } while (next_permutation(combo, combo + COMP_C));
    }
    // ********************************************************************************
      ...
      for (st_c1 = 1; st_c1 <= diff_c; st_c1++)
    {
        ...
        {
            st_c3 = cf - st_c1 - st_c2;
            if (st_c3 < 1 || st_c3 > st_c2) continue;
            ...
            for (int c = 0; c < charge_count_c; c++)
            {
                ...
                int charges_c[] = {ch_c1, ch_c2, ch_c3};
                ...
                // total charge for cations
                 int sum_c = st_c1*ch_c1 + st_c2*ch_c2 + st_c3*ch_c3;
