...
int local_balance=0; // target local balance
...
// ********************************************** INPUT FILE **************************************
      ...
      cin >> file_number;
      sscanf(file_number, "%*d_%d_%d", &local_balance, &delta);

      // Form the filename
      sprintf(filename, "for_generator_%s.txt", file_number);
...
if (fgets(line, sizeof(line), Fpdat) == NULL) {
    cout << "Error: Cannot read data from file" << endl;
    fclose(Fpdat);
    fclose(Fout);
    return 1;
}
...
if ( count_ch0>max_ch0 || count_ch1>max_ch1 || count_ch2>max_ch2 || count_ch3>max_ch3 || count_ch4>max_ch4 || count_ch5>max_ch5 || count_ch6>max_ch6 || count_ch7>max_ch7 ) continue;
  ...
  var_found++;    
  fprintf(Fout, "%d %d %d %d %d %d %d %d ", zk[p1], zk[p2], zk[p3], zk[p4], zk[p5], zk[p6], zk[p7], zk[p8]);
  fprintf(Fout, "%d %d %d %d %d %d %d %d ", zk[p9], zk[p10], zk[p11], zk[p12], zk[p13], zk[p14], zk[p15], zk[p16]);
  fprintf(Fout, "%d %d %d %d %d %d %d %d ", zk[p17], zk[p18], zk[p19], zk[p20], zk[p21], zk[p22], zk[p23], zk[p24]);
